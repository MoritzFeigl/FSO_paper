#
# DDS Optimization in function space
#
...
objective_function <- function(point, Test_number, 
  ...
  spatial_predictor_check <- sum(
    sapply(functions_splitted, function(x) sum(x %in% names(spatial_predictors)) < 1)
    )
           ...
           rbind(result_tracker_df,
                 data.frame(best_x1 = result_tracker_df[nrow(result_tracker_df), "best_x1"],
                            ...
                            n_iterations_since_BF_change = result_tracker_df[
                              nrow(result_tracker_df), "n_iterations_since_BF_change"] + 1,
                            stringsAsFactors = FALSE)),
           envir = .GlobalEnv)
    return(list(loss = NA, "Current point in function space" = point))
  }
