# FSO with DDS optimization for d-GR4J
# Moritz Feigl, 2019
#
...
if(!exists("true_para_field_df")) true_para_field_df <- true_para_field(Test_number)
...
objective_function <- function(point, Test_number, 
                               ...
                               para, para_1km, training_basins, true_state_list){
  latent_dim <- 6
    ...
    assign("result_tracker_df",
           rbind(result_tracker_df,
                 data.frame(best_x1 = result_tracker_df[nrow(result_tracker_df), "best_x1"],
                            ...
                            model_loss = result_tracker_df[nrow(result_tracker_df), "model_loss"],
                            x1 = point_tf[[1]],
                            x2 = point_tf[[2]],
                            x3 = point_tf[[3]],
                            n_iteration_used = 0,
  ...
  return(list(loss = full_loss, "Current point in function space" = point))
}
...
# Function space dds test
xBounds.df <- data.frame(lower = rep(-5, 18),
                         upper = rep(5, 18))

optim_dds <- try(dds_fs(xBounds.df = xBounds.df, OBJFUN = objective_function,
                        numIter = iterations, Test_number = Test_number, 
                        search_dim = 18, 
                        true_para_field_df = true_para_field_df,
...
while(class(optim_dds) == "try-error"){
  assign("result_tracker_df", data.frame(best_x1 = "init",
                                         best_x3 = "init",
                                         best_x4 = "init",
                                         full_loss = -9999.0,
...
feather::write_feather(data.frame(result_tracker_df, method = "dds", n_runs = runs, stringsAsFactors = FALSE),
                       paste0("Test ", substr(Test_number, 1, 1),"/",
                              "Test ", Test_number, "/DDS_GR4J_optimization_", 
